...
export function getSystemPromptFilePath(cwd: string, mode: Mode): string {
	return path.join(cwd, ".allytopic", `system-prompt-${mode}`)
}

/**
 * Loads custom system prompt from a file at .allytopic/system-prompt-[mode slug]
...
/**
 * Ensures the .allytopic directory exists, creating it if necessary
 */
export async function ensureRooDirectory(cwd: string): Promise<void> {
	const rooDir = path.join(cwd, ".allytopic")
