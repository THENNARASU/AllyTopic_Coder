...
describe("RooIgnoreController", () => {
	...
	describe("initialization", () => {
		/**
		 * Tests the controller initialization when .allytopicignore exists
		 */
		it("should load .allytopicignore patterns on initialization when file exists", async () => {
			// Setup mocks to simulate existing .allytopicignore file
			...
			expect(mockFileExists).toHaveBeenCalledWith(path.join(TEST_CWD, ".allytopicignore"))
			expect(mockReadFile).toHaveBeenCalledWith(path.join(TEST_CWD, ".allytopicignore"), "utf8")
		...
		/**
		 * Tests the controller behavior when .allytopicignore doesn't exist
		 */
		it("should allow all access when .allytopicignore doesn't exist", async () => {
			// Setup mocks to simulate missing .allytopicignore file
		 ...
		 */
		it("should set up file watcher for .allytopicignore changes", async () => {
			// Check that watcher was created with correct pattern
			expect(vscode.workspace.createFileSystemWatcher).toHaveBeenCalledWith(
				expect.objectContaining({
					base: TEST_CWD,
					pattern: ".allytopicignore",
				}),
			)
		 ...
		 */
		it("should handle errors when loading .allytopicignore", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith("Unexpected error loading .allytopicignore:", expect.any(Error))
	...
	describe("validateAccess", () => {
		...
		/**
		 * Tests the default behavior when no .allytopicignore exists
		 */
		it("should allow all access when no .allytopicignore content", async () => {
			// Create a new controller with no .allytopicignore
			mockFileExists.mockResolvedValue(false)
	...
	describe("validateCommand", () => {
		...
		/**
		 * Tests behavior when no .allytopicignore exists
		 */
		it("should allow all commands when no .allytopicignore exists", async () => {
			// Create a new controller with no .allytopicignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new RooIgnoreController(TEST_CWD)
			await emptyController.initialize()
	...
	describe("getInstructions", () => {
		/**
		 * Tests instructions generation with .allytopicignore
		 */
		it("should generate formatted instructions when .allytopicignore exists", async () => {
			// Setup .allytopicignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**")
			await controller.initialize()
		 ...
		 * Tests behavior when no .allytopicignore exists
		 */
		it("should return undefined when no .allytopicignore exists", async () => {
			// Setup no .allytopicignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
	...
	describe("file watcher", () => {
		/**
		 * Tests behavior when .allytopicignore is created
		 */
		it("should reload .allytopicignore when file is created", async () => {
			// Setup initial state without .allytopicignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
			...
			// Create and initialize controller with no .allytopicignore
			controller = new RooIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Force reload of .allytopicignore content manually
			await controller.initialize()
		...
		/**
		 * Tests behavior when .allytopicignore is changed
		 */
		it("should reload .allytopicignore when file is changed", async () => {
			// Setup initial state with .allytopicignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
		...
		/**
		 * Tests behavior when .allytopicignore is deleted
		 */
		it("should reset when .allytopicignore is deleted", async () => {
			// Setup initial state with .allytopicignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
