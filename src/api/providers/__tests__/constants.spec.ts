...
describe("DEFAULT_HEADERS", () => {
	...
	it("should have correct HTTP-Referer value", () => {
		expect(DEFAULT_HEADERS["HTTP-Referer"]).toBe("https://github.com/THENNARASU/AllyTopic_Coder")
	})

	it("should have correct X-Title value", () => {
		expect(DEFAULT_HEADERS["X-Title"]).toBe("AllyTopic Coder")
	})

	it("should have correct User-Agent format", () => {
		const userAgent = DEFAULT_HEADERS["User-Agent"]
		expect(userAgent).toBe(`AllyTopicCoder/${Package.version}`)
