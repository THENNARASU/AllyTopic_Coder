...
describe("SimpleInstaller", () => {
	...
	describe("installMode", () => {
		...
		it("should install mode when .allytopicmodes file does not exist", async () => {
			...
			expect(result.filePath).toBe(path.join("/test/workspace", ".allytopicmodes"))
		...
		it("should install mode when .allytopicmodes contains valid YAML", async () => {
		...
		it("should handle empty .allytopicmodes file", async () => {
			// Empty file content
		...
		it("should handle .allytopicmodes file with null content", async () => {
		...
		it("should handle .allytopicmodes file without customModes property", async () => {
		...
		it("should throw error when .allytopicmodes contains invalid YAML", async () => {
			...
			await expect(installer.installItem(mockModeItem, { target: "project" })).rejects.toThrow(
				"Cannot install mode: The .allytopicmodes file contains invalid YAML",
			)
	...
	describe("installMcp", () => {
		...
		it("should install MCP when mcp.json file does not exist", async () => {
			...
			expect(result.filePath).toBe(path.join("/test/workspace", ".allytopic", "mcp.json"))
			expect(mockFs.writeFile).toHaveBeenCalled()
		...
		it("should throw error when mcp.json contains invalid JSON", async () => {
			...
			await expect(installer.installItem(mockMcpItem, { target: "project" })).rejects.toThrow(
				"Cannot install MCP server: The .allytopic/mcp.json file contains invalid JSON",
			)
	...
	describe("removeMode", () => {
		...
		it("should throw error when .allytopicmodes contains invalid YAML during removal", async () => {
			const invalidYaml = "invalid: yaml: content: {"

			mockFs.readFile.mockResolvedValueOnce(invalidYaml)

			await expect(installer.removeItem(mockModeItem, { target: "project" })).rejects.toThrow(
				"Cannot remove mode: The .allytopicmodes file contains invalid YAML",
			)
		...
		it("should handle empty .allytopicmodes file during removal", async () => {
			// Empty file content
			mockFs.readFile.mockResolvedValueOnce("")
			mockFs.writeFile.mockResolvedValueOnce(undefined as any)
		...
		it("should handle .allytopicmodes file with null content during removal", async () => {
			// File exists but yaml.parse returns null
			mockFs.readFile.mockResolvedValueOnce("---\n")
			mockFs.writeFile.mockResolvedValueOnce(undefined as any)
		...
		it("should handle .allytopicmodes file without customModes property during removal", async () => {
			// File has valid YAML but no customModes property
			const contentWithoutCustomModes = yaml.stringify({ someOtherProperty: "value" })
			mockFs.readFile.mockResolvedValueOnce(contentWithoutCustomModes)
			mockFs.writeFile.mockResolvedValueOnce(undefined as any)
