...
export class WebAuthService extends EventEmitter<AuthServiceEvents> implements AuthService {
	...
	public async login(): Promise<void> {
		try {
			...
			const publisher = packageJSON?.publisher ?? "AllyTopic"
			const name = packageJSON?.name ?? "allytopic-coder"
			...
			this.log(`[auth] Error initiating AllyTopic Coder Cloud auth: ${error}`)
			throw new Error(`Failed to initiate AllyTopic Coder Cloud authentication: ${error}`)
		}
	}

	/**
	 * Handle the callback from AllyTopic Coder Cloud
	 *
	 * This method is called when the user is redirected back to the extension
	 * after authenticating with AllyTopic Coder Cloud.
	 *
	...
	public async handleCallback(
		...
		if (!code || !state) {
			vscode.window.showInformationMessage("Invalid AllyTopic Coder Cloud sign in url")
			return
		}
		...
		try {
			...
			vscode.window.showInformationMessage("Successfully authenticated with AllyTopic Coder Cloud")
			this.log("[auth] Successfully authenticated with AllyTopic Coder Cloud")
		} catch (error) {
			this.log(`[auth] Error handling AllyTopic Coder Cloud callback: ${error}`)
			const previousState = this.state
			this.state = "logged-out"
			this.emit("logged-out", { previousState })
			throw new Error(`Failed to handle AllyTopic Coder Cloud callback: ${error}`)
		}
	}
	...
	public async logout(): Promise<void> {
		...
		try {
			...
			vscode.window.showInformationMessage("Logged out from AllyTopic Coder Cloud")
			this.log("[auth] Logged out from AllyTopic Coder Cloud")
		} catch (error) {
			this.log(`[auth] Error logging out from AllyTopic Coder Cloud: ${error}`)
			throw new Error(`Failed to log out from AllyTopic Coder Cloud: ${error}`)
		}
	}
